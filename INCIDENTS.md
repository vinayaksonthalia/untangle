...
**Measured.** Razorpay recall 0.841 → **0.965** (0.821 → **0.946** sealed), precision still **1.000**, decoy false-positives still **0**. 109 Razorpay attributed (up from 95), abstentions down to 8. The recall is on par with the pre-proof-gate 0.938 — but every point of it rests on a real settlement tie, not resemblance. The reconciled slice (91) and recoverable ITC (₹43,200.99) are unchanged; full entity-level reconciliation of split groups (the per-credit reconcile model can't net a group yet) is the tracked next step.
