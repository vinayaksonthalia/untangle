...
| V4 | **Sample data is geo-templated — infer no fee/tax semantics from it** | Two independently-fetched Razorpay sources — the API-reference docs page and the `razorpay-node` SDK fixture (both in `fixtures/`) — carry byte-identical amounts (payment `fee 2900, tax 0`; transfer `fee 296, tax 46`). The docs page is geo-served by client egress IP (one fetch returned INR/MasterCard, another USD/AMEX), so currency/network strings vary while amounts do not (see INCIDENTS 001). Verified by `scripts/verify_schema_claims.py` | `tax` is documented as "the tax on the fee"; transfer row `debit 100296 = amount 100000 + fee 296` (not +tax) proves tax is **inside** fee — one consistent semantic. The `tax: 0` on the payment row is placeholder sample data, NOT a second semantic. **Do not claim a schema inconsistency here.** INR production semantics to be confirmed against a real merchant recon report. |
